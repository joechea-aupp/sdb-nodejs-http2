...
const updateBook = (stream, headers) => {
  ...
  stream.on("end", () => {
    ...
    const bookIndex = books.findIndex((book) => book.title === body.title);

    if (bookIndex === -1) {
      stream.respond({ ":status": 404 });
      stream.end("Book not found");
    }

    books[bookIndex] = { ...body };
    stream.respond({ ":status": 200, "Content-Type": "application/json" });
    stream.end(`Book updated ${JSON.stringify(books[bookIndex])}`);
    console.log(books);
  });
};
...
const deleteBook = (stream, headers) => {
  ...
  stream.on("end", () => {
    ...
    books = books.filter((book) => book.title !== body.title);
    stream.respond({ ":status": 200, "Content-Type": "application/json" });
    stream.end(`Book deleted ${JSON.stringify(body)}`);
    console.log(books);
  });
};
